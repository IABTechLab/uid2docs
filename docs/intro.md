...
| [Workflow for Publishers](overviews/overview-publishers.md#workflow-for-publishers) | Organizations that propagate UID2 tokens to the bid stream via SSPs.<br/> NOTE: Publishers can choose to leverage the UID2 SDK for JavaScript (see [JavaScript Express Integration Guide](publisher-client-side.md) or [UID2 SDK for JavaScript Reference Guide](sdks/client-side-identity.md)) or complete their own server-only integration without using an SDK. | [JavaScript Express Integration Guide](publisher-client-side.md)<br/>[Standard JavaScript Integration Guide](guides/integration-javascript-standard.md)<br/>[Publisher Integration Guide, Server-Only](guides/custom-publisher-integration.md) |
