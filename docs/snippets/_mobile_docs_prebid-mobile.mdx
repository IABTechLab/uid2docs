...
This section is for participants who want to integrate with UID2 and use the [Prebid Mobile SDK](https://prebid.org/product-suite/prebid-mobile/) for header bidding in iOS and Android applications.
...
1. Set up the Prebid Mobile SDK for Android or iOS, following the steps in the applicable Prebid integration guide:
...
2. Add the UID2 Mobile SDK to your app: follow the steps in [Add the UID2 Mobile SDK to Your Mobile App](#add-the-uid2-mobile-sdk-to-your-mobile-app).
...
3. After adding the dependency, you'll need to configure the integration. First initialize the `UID2Manager`, and then Prebid, as shown in the following examples.
...
4. Configure a callback that's passed in during initialization.
