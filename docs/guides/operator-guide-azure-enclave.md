...
| :--- | :--- | :--- | :--- |
| E12 | Data Download Failure | Look for log messages containing `"Cannot download required files"` along with `"Failed to load"` errors from `RotatingStoreVerticle`. These will include HTTP status codes (e.g., `"HTTP response code 403"`) or exception types (e.g., `"exception: RequestTimeoutException"`). | Check the HTTP status code or exception in the error message: <br/>**404 errors** - Verify operator key is valid and configuration paths are correct.<br/>**403 errors** - Check managed identity permissions for storage access and verify operator credentials.<br/>**Timeout errors** - Verify network connectivity, check NSG/firewall rules allow outbound HTTPS, and ensure UID2 service endpoints are accessible.<br/>**500 errors** - Temporary UID2 service issue, retry or contact UID2 support if persistent. |