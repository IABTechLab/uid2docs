...
- **トークンは必要な場合のみローテーションさせる**
  ...
  UID2 Token は、ユーザーの HEM (Home Entertainment Media) やスマートフォンに紐付けられており、視聴セッションやアプリセッションには紐付けられていません。ユーザーに有効な UID2 Token がある限り、新しい視聴セッションやアプリセッションごとに新しい UID2 を生成する必要はありません。たとえば、ユーザーがアプリを離れ、再度アプリを開いた場合、既存のトークンがまだ有効であれば、新しい UID2 を生成する必要はありません。
