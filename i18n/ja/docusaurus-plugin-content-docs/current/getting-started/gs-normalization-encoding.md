...
メールアドレスハッシュは、正規化されたメールアドレスの SHA-256 ハッシュを Base64 エンコードしたものです。メールアドレスはまず正規化され、次に SHA-256 ハッシュアルゴリズムを使ってハッシュ化され、その結果のハッシュ値のバイトが Base64 エンコーディングを使ってエンコードされます。

:::note
- ハッシュする前に、正規化されたメールが UTD-16 のような他のエンコーディングシステムではなく、UTF-8 であることを確認してください。
- ハッシュ値のバイトは Base64 エンコードされていて、16 進エンコードされた文字列表現ではありません。
:::
...
電話番号ハッシュは、正規化された電話番号の SHA-256 ハッシュを Base64 エンコードしたものです。電話番号はまず正規化され、次に SHA-256 ハッシュアルゴリズムを使ってハッシュ化され、その結果のハッシュ値のバイトが Base64 エンコーディングを使ってエンコードされます。

:::note
- ハッシュする前に、正規化された電話番号が UTD-16 のような他のエンコーディングシステムではなく、UTF-8 であることを確認してください。
- ハッシュ値のバイトは Base64 エンコードされていて、16 進エンコードされた文字列表現ではありません。
:::
